...
const QUESTION_HISTORY_KEY = "beef_cut_fit_compare_question_history_v1";
const QUESTION_LAST_SET_KEY = "beef_cut_fit_compare_last_set_v1";
...
const compareLiveTopCut = document.getElementById("compareLiveTopCut");
const compareSandboxTopCut = document.getElementById("compareSandboxTopCut");
const compareLiveTopList = document.getElementById("compareLiveTopList");
const compareSandboxTopList = document.getElementById("compareSandboxTopList");
const compareDeltaList = document.getElementById("compareDeltaList");
...
function showResults() {
  ...
  const liveRankedCuts = rankCuts(profile, signals);
  const proposedRankedCuts = rankCutsWithProposedMatrix(liveRankedCuts);
  const rankedCuts = liveRankedCuts;
  const primary = liveRankedCuts[0];
  const second = liveRankedCuts[1];
  ...
  renderCompareResults(liveRankedCuts, proposedRankedCuts);
...
function rankCutsWithProposedMatrix(fallbackRankedCuts = []) {
  const matrixByGroup = window.PROPOSED_PRIORITY_MATRIX?.by_group;
  if (!matrixByGroup) {
    return fallbackRankedCuts;
  }

  const scoreByCutId = CUTS.reduce((acc, cut) => {
    acc[cut.id] = 0;
    return acc;
  }, {});

  for (let questionIndex = 0; questionIndex < state.questionSet.length; questionIndex += 1) {
    const selectedOptionIndex = state.answers[questionIndex];
    if (selectedOptionIndex === null || selectedOptionIndex === undefined) {
      continue;
    }

    const group = state.questionSet[questionIndex]?.group;
    if (!group) {
      continue;
    }

    const answerOrder = String(selectedOptionIndex + 1);
    const matrixRow = matrixByGroup[group]?.answers?.[answerOrder];
    if (!matrixRow) {
      continue;
    }

    CUTS.forEach((cut) => {
      const value = Number(matrixRow[cut.name] || 0);
      scoreByCutId[cut.id] += value;
    });
  }

  return CUTS.map((cut) => ({
    cut,
    score: scoreByCutId[cut.id] || 0,
  })).sort((a, b) => {
    if (b.score !== a.score) {
...
function renderCompareResults(liveRankedCuts, proposedRankedCuts) {
  if (
    !compareSummary ||
    !compareLiveTopCut ||
    !compareSandboxTopCut ||
    !compareLiveTopList ||
    !compareSandboxTopList ||
    !compareDeltaList
  ) {
    return;
  }

  const liveTop = liveRankedCuts[0];
  const proposedTop = proposedRankedCuts[0];
  if (!liveTop || !proposedTop) {
    compareSummary.textContent = "Comparison unavailable.";
    return;
  }

  compareLiveTopCut.textContent = `Live Top Cut: ${liveTop.cut.name}`;
  compareSandboxTopCut.textContent = `Proposed Top Cut: ${proposedTop.cut.name}`;

  renderCompareList(compareLiveTopList, liveRankedCuts, "live");
  renderCompareList(compareSandboxTopList, proposedRankedCuts, "proposed");

  const topN = 5;
  const liveTopIds = new Set(liveRankedCuts.slice(0, topN).map((entry) => entry.cut.id));
  const proposedTopIds = new Set(
    proposedRankedCuts.slice(0, topN).map((entry) => entry.cut.id)
  );
  const overlapCount = [...liveTopIds].filter((id) => proposedTopIds.has(id)).length;

  const topChanged = liveTop.cut.id !== proposedTop.cut.id;
  const matrixReady = Boolean(window.PROPOSED_PRIORITY_MATRIX?.by_group);
  compareSummary.textContent = matrixReady
    ? `${topChanged ? "Top cut changed." : "Top cut stayed the same."} Top-${topN} overlap: ${overlapCount}/${topN}.`
    : "Proposed matrix not loaded. Showing live results on both sides.";

  renderCompareDeltas(compareDeltaList, liveRankedCuts, proposedRankedCuts);
}

function renderCompareList(target, rankedCuts, label) {
  target.innerHTML = "";
  rankedCuts.slice(0, 5).forEach((entry, index) => {
    const listItem = document.createElement("li");
    listItem.textContent = `#${index + 1} ${entry.cut.name} (${label} score ${entry.score})`;
    target.appendChild(listItem);
  });
}

function renderCompareDeltas(target, liveRankedCuts, proposedRankedCuts) {
  target.innerHTML = "";
  const liveRankMap = buildRankMap(liveRankedCuts);
  const proposedRankMap = buildRankMap(proposedRankedCuts);

  const changes = CUTS.map((cut) => {
    const liveRank = liveRankMap[cut.id] || CUTS.length;
    const proposedRank = proposedRankMap[cut.id] || CUTS.length;
    return {
      cutName: cut.name,
      liveRank,
      proposedRank,
      delta: proposedRank - liveRank,
      absDelta: Math.abs(proposedRank - liveRank),
    };
  })
    .filter((entry) => entry.absDelta > 0)
    .sort((a, b) => {
  ...
  if (changes.length === 0) {
    addListItem(target, "No rank deltas between live and proposed outputs.");
    return;
  }
  ...
  changes.forEach((change) => {
    ...
    addListItem(
      target,
      `${change.cutName}: live #${change.liveRank} -> proposed #${change.proposedRank} (${Math.abs(change.delta)} ${direction})`
    );
  });
}

function buildRankMap(rankedCuts) {
  return rankedCuts.reduce((acc, entry, index) => {
    acc[entry.cut.id] = index + 1;
    return acc;
  }, {});
}
